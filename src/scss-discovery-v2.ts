...
export class ScssDiscoveryEngine {
  ...
  private async scanForScssFiles(
    ...
    try {
      ...
      // Use micromatch for filtering instead of glob's ignore
      for (const includePattern of this.config.includePatterns) {
        try {
          const pattern = path.join(rootPath, includePattern).replace(/\\/g, '/');
          
          // Simple glob call without ignore parameter 
          const foundFiles = await glob(pattern, {
            absolute: true,
            nodir: true
          });
          
          // Filter out excluded patterns manually using micromatch
          const filteredFiles = foundFiles.filter(file => {
            const relativePath = path.relative(rootPath, file);
            return !micromatch.isMatch(relativePath, this.config.excludePatterns);
          });
          ...
          errors.push({
            type: DiscoveryErrorType.FILE_ACCESS,
            message: `Failed to scan pattern ${includePattern}: ${(error as Error).message}`,
            branch: branchName,
            details: { pattern: includePattern, error }
          });
        }
      }
